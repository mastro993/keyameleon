...
section with the full tag comparison and change list. The notes list no
contributors of their own: the release page renders its own Contributors card
from the `@login` mentions in the notes, and a hand-written list duplicated
that card with square avatars. Keyameleon does not publish a `CHANGELOG.md`
file or a changelog artifact.
